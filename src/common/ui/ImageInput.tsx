...
const ImageInput: React.FC<IImageInputProps> = ({value, ...props}) => {
    const [src, setSrc] = useState(value || '');

    const changeHandler =  (e: any) => {
        const file = e.target.files[0];
        api.postImage(file).then((src) => setSrc(`${process.env.REACT_APP_API_URL}${src}`));
    };
...
useEffect(()=>{},[src])
    ...
    return (
            <Wrapper>
                <StyledInputImage
                    style={{backgroundSize: 'contain', background: `url(${src}) center center no-repeat`,}}>
                    <input onChange={changeHandler}
                           accept="image/png, image/jpeg"   type="file" hidden/>
                    <input type='text'{...props} value={src} hidden/>
                </StyledInputImage>
            </Wrapper>
    )
};
